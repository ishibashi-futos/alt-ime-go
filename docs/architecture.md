...
3. **単独 Alt のメニュー動作は既定で二段抑制する。** 空打ち候補の Alt down 時に
   ...
   届くため、定数 `config.SuppressAltMenuFocus` で無効化可能にし、アプリ横断試験を必須とする。
...
- `LLKHF_INJECTED` かつ `dwExtraInfo == win32.OwnInputTag` の場合だけ自己注入として状態機械から除外する。
...
`win32.OwnInputTag` は 64-bit 固定値とし、IME 入力とメニュー抑制入力の全 `KEYBDINPUT.dwExtraInfo` に設定する。
これは所有権判定用であり、セキュリティ境界ではない。

### 4.2 Alt メニューフォーカス抑制

- 機能が有効で、他キーが down でない空打ち候補として `tracking` を開始し、かつ
  `config.SuppressAltMenuFocus == true` の場合、物理 Alt の初回 down で未割当 VK `0x07` の down/up を
  `SendInput` する。全イベントに `win32.OwnInputTag` を付ける。既押下キーがある chord には送らない。
...
### 4.3 空打ち状態機械 `TapMachine`（internal/hookstate/tapstate.go）
...
- `config.TapMaxHoldMs` は既定 500ms とし、0（無期限）は採用しない。
...
### 4.5 Enter送信ガード `GuardMachine`（internal/hookstate/enterguard.go）

対象アプリでの誤送信を防ぐため、Enter の意味を置換する。空打ち機械とは独立した
第二の状態機械（純粋ロジック）として実装し、同じイベント列を
`TapMachine.Feed` の後に食わせる。**callback はガード対象の Enter を消費するだけ**で、
置換キーの選択と注入は IME 切替と同じ二段配送で UI スレッドが行う
（`msgHookDispatchGuard` → `msgGuardEnter`。WPARAM に send/composing ビット、
LPARAM に対象 HWND）。IME 状態の問い合わせは有界とはいえ外部 HWND への同期呼出しで
あり、フック callback にもフックスレッドのポンプにも置けないためである。

**消費条件（すべて AND）:**

1. フック全体が有効、かつ Enter送信ガードが有効（トレイでトグル、フックスレッドが所有）。
2. 前面ウィンドウのプロセス exe 名が `config.MatchGuardTarget`（`internal/config/tunables.go` の対象 exe リスト） に一致（キャッシュ判定）。
3. 物理（非注入）の `VK_RETURN` down。他プロセス注入の Enter は素通し。テンキー Enter も対象。
4. 修飾が「なし」または「Ctrl のみ」。Shift / Alt / Win のいずれかが down なら介入しない。

**状態機械（`idle` / `swallow` の 2 状態 + 修飾キー down 集合 + 変換中ビリーフ）:**

| 現状態 | 入力 | 次状態 / 動作 |
|---|---|---|
| idle | 物理 Enter down、消費条件成立 | swallow。ブロックし、`(send=Ctrl押下, composing)` を UI へ二段配送 |
| idle | Enter down（Shift/Alt/Win を含む、非対象、無効、注入） | idle のまま素通し |
| idle | Enter up | idle のまま素通し（down を素通しした押下、resync で孤立した up） |
| swallow | 物理 Enter down（auto-repeat） | swallow のままブロック。再配送しない |
...
**変換中ビリーフ（CON-9 ヒューリスティック）:** 他プロセスの変換中状態を確実に検出する
公式 API はないため、状態機械がキー列から「変換中の疑い」を推定する。

- 文字系キー down（英字・数字・OEM 記号。テンキーと Space は除く）で `composing = true`。
- Enter down（消費・素通し・他プロセス注入を問わず）、Escape、IME 切替キー
  （かな/漢字/`VK_IME_ON/OFF`/半角全角）、フォーカス変更（WinEvent）、resync で `composing = false`。
- Space（変換）・Backspace・矢印・修飾キーはビリーフを変えない（変換は開いたまま）。
- 誤りの方向を非対称に設計する: **stale-true は fail-open**（素の Enter を届ける =
  アプリ既定の動作に戻るだけ）だが、stale-false は変換確定を壊す。そのためクリア条件は
  「確実に変換を終わらせるイベント」に限定する。マウスクリックでの確定は見えないため、
  直後の Enter は送信になり得る（既知の残存リスク）。

**置換の選択（UI スレッド、`onGuardEnter`）:**

1. 再検証: 終了中でなく、有効かつガード有効、`IsWindow(target)`、
   `GetForegroundWindow() == target`。不一致なら破棄する（別アプリへの注入の方が
   1 打鍵の喪失より有害）。
2. `send`（Ctrl 押下）なら素の Enter を届ける（送信）。
3. `composing` なら IME open 状態を問い合わせ、**open なら素の Enter を届けて変換確定を
   維持する**。問い合わせ先は前面トップレベルではなく**実際のフォーカスウィンドウ**:
   `GetGUIThreadInfo(0)`（前面スレッド）の `hwndFocus` を解決し（失敗時は対象スレッド→
   対象 HWND へフォールバック）、その `ImmGetDefaultIMEWnd` へ `IMC_GETOPENSTATUS` を
   `SendMessageTimeout`（`config.Imm32TimeoutMs`）で送る。既定 IME ウィンドウは**スレッド単位**で、
   WebView2/Chromium ホストではフォーカス先の子ウィンドウが別スレッド（別プロセス）に
   属するため、トップレベルへ問い合わせると変換中でも closed が返る。
   **答えが得られない場合（フォーカス/IME ウィンドウなし、タイムアウト、UIPI）も素の
   Enter を届ける**: 変換確定を壊すより、アプリ既定の Enter 動作へ戻る方が安全（fail-open）。
   明確に closed（英数直接入力）と答えが得られた場合だけ通常の置換に進む。
4. それ以外はタグ付き Shift+Enter を注入する（改行）。

ガードの注入キーには `MapVirtualKeyW` による実スキャンコードを付与する。Chromium/WebView2 は
DOM の `code` 値をスキャンコードから導出するため、`wScan=0` の注入 Enter を無視する Web
キーハンドラがあり得る（メニュー抑制・IME キーの既存注入は従来どおり変更しない）。
`config.GuardTrace`（既定 true、実機検証用）で置換判定 1 件ごとの内訳
（send/composing/IME 応答/選択結果）を `OutputDebugStringW` へ記録する。

素の Enter の届け方: `GetAsyncKeyState` で物理押下中の左右 Ctrl を特定し、押下中の側だけを
一時解放→Enter→復元の順でタグ付き注入する（Ctrl+Enter を素の Enter に見せる）。どちらも
up ならそのまま Enter の down/up 対を注入する。挿入数不足時はタグ付き key-up/down で
best-effort 復旧し、UI スレッドなので直接 `OutputDebugStringW` へ記録する。
注入はすべて `win32.OwnInputTag` 付きの 1 回の `SendInput` で行い、自己注入フィルタにより
両状態機械から除外されるため再帰しない。

resync（起動・有効化・ガードトグル・セッション/電源復帰）は修飾集合を `GetAsyncKeyState`
から再構築し、ビリーフをクリアして idle へ戻す。swallow 中に resync した押下の孤立 up は
idle が無視するので無害。

**空打ち機械との排他:** Enter イベントはブロック有無に関係なく `TapMachine.Feed` にも先に食わせ、
...
**タイミング上のトレードオフ:** 置換注入は物理 Enter の数 ms 後になるため、その間に打鍵された
物理キーと順序が入れ替わり得る（人間の打鍵間隔より十分短く、実害は想定しない）。UI 再検証で
破棄された場合、その Enter は完全に失われる（対象競合防止と同じ設計判断）。
...
- `assets/alt-ime-icon.ico` の 16 / 20 / 24 / 32 / 40 / 48 / 64 / 128 / 256px PNG を
  ...
  `cmd/alt-ime-go/rsrc_windows_amd64.syso` に埋め込む。トレイは実行モジュールの ID 1 を
  読み込み、exe と意匠を統一する。
- アイコン原稿は墨色 `#303236` と白 `#f7f7f2` のキーキャップ形状とし、
  `go run ./tools/mkicon` → `go run ./tools/mkrsrc` の順で標準ライブラリだけを使って再生成する。
...
Enter送信ガード（§4.5）はこの作法の実装例である: 独立状態機械
（internal/hookstate/enterguard.go）を追加し、排他仕様（Alt down 中の Enter に介入しない、
Enter はタップキャンセル要因のまま）を実装前に確定した。置換の選択と注入は IME 切替と同じ
二段配送（`msgHookDispatchGuard` → `msgGuardEnter`）で UI スレッドが行う。
