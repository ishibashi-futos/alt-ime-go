...
3. `composing` なら `ImmGetDefaultIMEWnd(target)` へ `IMC_GETOPENSTATUS` を
   `SendMessageTimeout`（`imm32TimeoutMs`）で問い合わせ、**IME が open なら素の Enter を
   届けて変換確定を維持する**。closed（英数直接入力）や問い合わせ失敗なら通常の置換に進む。
